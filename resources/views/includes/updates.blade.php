...
@if($response->is_plus_18 == 1)
	@if(\App\Helper::check_is_18(\Auth::user()->date_of_birth) >= 18)
	<div class="card mb-3 card-updates" data="{{$response->id}}">
		<div class="card-body">
			<div
				class="pinned_post text-muted small w-100 mb-2 {{ $response->fixed_post == '1' && request()->path() == $response->user()->username ? 'pinned-current' : 'display-none' }}">
				<i class="fa fa-thumbtack mr-2"></i> {{ trans('general.pinned_post') }}
			</div>
			<div class="media">
				<span class="rounded-circle mr-3">
					<a href="{{url($response->user()->username)}}">
						<img src="{{ Storage::url(config('path.avatar').$response->user()->avatar) }}"
							alt="{{$response->user()->name}}" class="rounded-circle avatarUser" width="60" height="60">
					</a>
				</span>
				...
				<div class="media-body">
					<h5 class="mb-0 font-montserrat">
						...
						<small class="verified" title="{{trans('general.verified_account')}}" data-toggle="tooltip"
							data-placement="top">
						...
						<button class="d-none copy-url" id="url{{$response->id}}"
							data-clipboard-text="{{url($response->user()->username.'/post', $response->id)}}">{{trans('general.copy_link')}}</button>
						...
						<div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdown_options">
							...
							<a class="dropdown-item"
								href="{{url($response->user()->username.'/post', $response->id)}}">{{trans('general.go_to_post')}}</a>
							@endif
							...
							<button class="dropdown-item"
								onclick="$('#url{{$response->id}}').trigger('click')">{{trans('general.copy_link')}}</button>
							<a class="dropdown-item"
								href="{{url('update/edit',$response->id)}}">{{trans('general.edit_post')}}</a>
							{!! Form::open([
							'method' => 'POST',
						...
						->where('stripe_id', '<>', '')
							...
							->count() != 0
							|| Auth::check() && Auth::user()->id != $response->user()->id && Auth::user()->role == 'admin'
							...
							<div class="modal fade" id="reportUpdate{{$response->id}}" tabindex="-1" role="dialog"
								aria-hidden="true">
								<div class="modal-dialog modal-danger modal-xs">
									<div class="modal-content">
										<div class="modal-header">
											<h6 class="modal-title font-weight-light" id="modal-title-default"><i
													class="fas fa-flag"></i> {{trans('admin.report_update')}}</h6>
										...
										<form method="POST" action="{{url('report/update', $response->id)}}"
											...
											<div class="modal-body">
												...
												<div class="form-group">
													...
													<select name="reason" class="form-control custom-select">
														...
														<option value="privacy_issue">{{trans('admin.privacy_issue')}}
														</option>
														<option value="violent_sexual_content">
															{{trans('admin.violent_sexual_content')}}</option>
													</select>
												</div><!-- /.form-group-->
											</div><!-- Modal body -->
											...
											<div class="modal-footer">
												<button type="submit"
													class="btn btn-xs btn-white">{{trans('admin.report_update')}}</button>
		...
		->count())}} --}}
			...
			->where('stripe_id', '<>', '')
				...
				<div class="btn-block">
					...
					@endphp
					<a href="{{ Storage::url(config('path.images').$response->image) }}"
						data-group="gallery{{$response->id}}" class="js-smartPhoto w-100">
						<img src="{{url('files/preview', $response->image)}}?w=100&h=100" data-src="{{$urlImg}}?w=650&h=650"
							class="img-fluid lazyload d-inline-block w-100" alt="{{ e($response->description) }}">
					</a>
					@endif
	
					@if($response->video != '')
					<video id="video-{{$response->id}}" class="js-player w-100 @if (!request()->ajax())invisible @endif"
						controls>
					...
					<div class="mx-3 border rounded">
						<audio id="music-{{$response->id}}" class="js-player w-100 @if (!request()->ajax())invisible @endif"
							controls>
				...
				<div class="card-footer bg-white border-top-0">
					<h4>
						@php
						$likeActive = Auth::check() && Auth::user()->likes()->where('updates_id',
						$response->id)->where('status','1')->first();
						$bookmarkActive = Auth::check() && Auth::user()->bookmarks()->where('updates_id',
						$response->id)->first();
						...
						->where('stripe_id', '<>', '')
							...
							<a href="javascript:void(0);"
								class="btnLike @if($likeActive)active @endif {{$buttonLike}} text-muted mr-2" @auth
								data-id="{{$response->id}}" @endauth>
								<i class="@if($likeActive)fas @else far @endif fa-heart"></i> <small><strong
										class="countLikes">{{Helper::formatNumber($response->likes()->count())}}</strong></small>
							</a>
							...
							<a href="javascript:void(0);"
								class="@if($bookmarkActive) text-primary @else text-muted @endif float-right {{$buttonBookmark}}"
					...
					<div class="container-media">
						...
						@php
						$comments = $response->comments()->take($settings->number_comments_show)->orderBy('id',
						'DESC')->get();
						$data = [];
					...
					->where('stripe_id', '<>', '')
						...
						<div class="media position-relative">
							...
							<span href="#" class="float-left">
								<img src="{{ Storage::url(config('path.avatar').auth()->user()->avatar) }}"
							...
							<div class="media-body">
								<form action="{{url('comment/store')}}" method="post" class="comments-form">
									...
									<input type="text" name="comment" class="form-control comments border-0"
										autocomplete="off" placeholder="{{trans('general.write_comment')}}">
							</div>
							</form>
						</div>
						@endif
