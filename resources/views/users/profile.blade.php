      ...
      <div class="col-lg-12">
          ...
          <div class="media-body">
            ...
            @endif
              ...
              @if (Auth::check() && Auth::user()->id != $user->id && $checkSubscription == 0 && ! $paymentIncomplete)
                <a href="javascript:void(0);" data-toggle="modal" data-target="#subscriptionForm" class="btn btn-primary btn-profile mr-1"><i class="fa fa-unlock-alt mr-1"></i> @if($user->promotion_price && Helper::check_promotion_end_date($user->promotion_end_date)
                )Get Access <span style="-webkit-text-decoration-line: line-through; text-decoration-line: line-through;">${{$user->price}}</span> ${{$user->promotion_price}}/mo @else{{trans('general.get_access_month', ['price' => Helper::amountFormatDecimal($user->price)])}}@endif</a>
        ...
        <ul class="nav nav-profile justify-content-center">
          <li class="nav-link @if (request()->path() == $user->username)active @endif">
            <small class="btn-block sm-btn-size">{{\App\Helper::check_is_18(\Auth::user()->date_of_birth) >= 18?  $user->updates()->count() : $user->updates()->where('is_plus_18',0)->count() }}</small>
          ...
          <div class="modal-body p-0">
            <div class="card bg-white shadow border-0">
              ...
              <div class="card-body px-lg-5 py-lg-5 position-relative">
                ...
                )Get Access <span style="-webkit-text-decoration-line: line-through; text-decoration-line: line-through;">${{$user->price}}</span> ${{$user->promotion_price}}/mo @else{{trans('general.get_access_month', ['price' => Helper::amountFormatDecimal($user->price)])}}@endif {{trans('general.unlocked_content')}} {{$user->name}}
                  </h6>
                </div>
                ...
                @if($user->price != '0.00' && $user->price != '0.0' && $user->price != '0' )
                ...
                <form method="post" action="@if($user->price != '0.00' && $user->price != '0.0' && $user->price != '0' ){{url('buy/subscription')}}@else{{route('buy-free')}}@endif" id="formSubscription">
                  ...
                  <div class="text-center">
                    <button type="submit" id="subscriptionBtn" class="btn btn-primary mt-4"><i></i> {{trans('general.pay')}} @if($user->promotion_price && Helper::check_promotion_end_date($user->promotion_end_date)
                      ){{Helper::amountWithoutFormat($user->promotion_price)}} @else{{Helper::amountWithoutFormat($user->price)}}@endif {{$settings->currency_code}}</button>
                  </div>
                </form>
