...
class MessagesController extends Controller
{
  ...
  public function send_bulk_message(Request $request){
    ...
    $messages = [
      ...
      'send_to.required' => 'Please select message send type',
    ...
    if($validator->fails()){
      ...
      if(request('send_to') == 'selectes'){

        if(!request('subscriber_ids')){ 
          $data['id'] = 'subscriber_ids_error';
          $data['custom_msg'] = 'Please select subscriber';
      ...
      if(request('send_to') == 'all'){
        $subs = array_unique(array_pluck(\Auth::user()->userSubscriptions, 'stripe_plan'));
        // dd($subs);
        $sub_users = User::whereIn('plan', $subs)->get(); 
        // dd($sub_users);
      }
      else{
        $sub_users = User::whereIn('id', request('subscriber_ids'))->get(); 
      }


      foreach($sub_users as $key => $temp_user){

      $conversation = Conversations::where('user_1', Auth::user()->id)
  				->where('user_2', $temp_user->id)
  				->orWhere('user_1', $temp_user->id)
        ...
        if (! isset($conversation)) {
          ...
          $newConversation->user_2 = $temp_user->id;
        ...
        if ($request->hasFile('photo')) {
            ...
            $message->conversations_id = $conversationID;
    				...
    				$message->to_user_id       = $temp_user->id;
            $message->message         = '';
            $message->file             = $file;
