  ...
  if (auth()->user()->verified_id == 'no' || auth()->user()->verified_id == 'reject') {
    ...
    $this->settings->min_subscription_amount = $this->settings->min_subscription_amount;
      ...
      $validator = Validator::make($input, [
        ...
        'welcome_message' => 'required|max:'.$this->settings->story_length.'',
        ...
        'price' => 'required_if:_verified_id,==,yes|numeric|min:0|max:'.$this->settings->max_subscription_amount.'',
        'countries_id' => 'required',
        'city' => 'max:100',
        'address' => 'max:100',
        'zip' => 'max:20',
     ...
     if ($validator->fails()) {
      ...
      if(request('price') >= 1){
        if(request('promotion_price') && request('promotion_price') < request('price')){
        $user->promotion_price = request('promotion_price','0.00');
        $user->promotion_end_date = request('promotion_end_date',null);
        }else{
          $user->promotion_price = 0.00;
          $user->promotion_end_date = null;
        }
      }
      else{
        $user->promotion_price = 0.00;
      ...
      $user->welcome_msg    = trim(Helper::checkTextDb($this->request->welcome_message));
      $user->save();
